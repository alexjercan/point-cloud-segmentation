...
def read_x_plyfile(plypath):
    ...
    coord = np.array(
        [plydata["points"]["x"], plydata["points"]["y"], plydata["points"]["z"]]
    ).T
    rgb = np.array([plydata["rgb"]["r"], plydata["rgb"]["g"], plydata["rgb"]["b"]]).T
    ...
    if os.path.isdir(path):
        ...
        if ext == ".ply":
            ...
            extract_dir = os.path.join(
                os.path.split(path)[0], os.path.split(path)[1][:-4]
            )
            ...
            for xfile in xfile_list:
                station_id, features = read_x_plyfile(os.path.join(extract_dir, xfile))
                x[station_id] = features
    return x
...
def save_ptcloud_withGT(x, y, station2index, savedir):
    ...
    for station in x:
        ...
        coord = np.array(
            [tuple(x) for x in coord], dtype=[("x", "f4"), ("y", "f4"), ("z", "f4")]
        )
        rgb = np.array(
            [tuple(x) for x in rgb], dtype=[("r", "u1"), ("g", "u1"), ("b", "u1")]
        )
